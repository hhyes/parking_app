...
def segmentation():
    plate_like_objects=cca2();
# The invert was done so as to convert the black pixel to white pixel and vice versa
    for x in plate_like_objects:
        license_plate = np.invert(x)
...
# this will eliminate some
        character_dimensions = (0.5*license_plate.shape[0], 0.90*license_plate.shape[0], 0.0*license_plate.shape[1], 0.3*license_plate.shape[1])
    ...
    #print(character_dimensions)
      ...
      #  print("%s %s" % (region_height, region_width))
        ...
        # draw a red bordered rectangle over the character.
            #rect_border = patches.Rectangle((x0, y0), x1 - x0, y1 - y0, edgecolor="red", linewidth=2, fill=False)
            #x1.add_patch(rect_border)

        # resize the characters to 20X20 and thax1.imshow(license_plate, cmap="gray")en append each character into the characters list
